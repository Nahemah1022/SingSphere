...
const EmptyRoomStory = () => {
  return (
    <div style={{ backgroundColor: 'rgba(0,0,0,0.1)', height: 400, width: 400 }}>
      <EmptyRoom />
    </div>
  );
};
...
const UserRemoteStory = () => {
  ...
  return (
    <div>
      <div>
        <label>
          <input type='checkbox' onChange={() => setIsMuted(!isMuted)} checked={isMuted} />
          muted
        </label>
      </div>
      <div style={{ backgroundColor: 'rgba(0,0,0,0.1)' }}>
...
const ButtonMicrohoneContainer = () => {
  ...
  return <ButtonMicrohone muted={isMuted} onClick={() => setIsMuted(!isMuted)} />;
};
...
const IconMicrophone: React.FC<{
  ...
  if (muted) {
    return (
      <svg aria-hidden='false' width='80%' height='80%' viewBox='0 0 24 24'>
        <path
