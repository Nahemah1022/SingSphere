    ...
    const handleSearch = async (option: string) => {
      try {
        ...
        const response = await axios.get(
          `https://zuooeb1uui.execute-api.us-east-1.amazonaws.com/Dev/GET/?song=${formattedTerm}`,
        );
    ...
    const queueSong = async (result: Song) => {
      try {
        ...
        let res = await axios.post(
          'https://zuooeb1uui.execute-api.us-east-1.amazonaws.com/Dev/final-music',
          base64RequestBody,
        );
        console.log(res);
          ...
          >
            <Box className={css.roomModal}>
              <div className={css.searchBar}>
                ...
                />
                <button className={css.searchArtist} onClick={() => handleSearch('artist')}>
                  Search Artist
                </button>
                <button className={css.searchSong} onClick={() => handleSearch('song')}>
                  Search Song
                </button>
                <button className={css.searchCategory} onClick={() => handleSearch('category')}>
                  Search Category
                </button>
              </div>
              <ColorRing
                      ...
                      <List>
                        {songs.map((result, index) => (
                          <ListItem key={index} className={css.resultItems}>
                            ...
                            <ListItemText
                              primary={capitalizeAndFormat(result.labels[0])}
                              className={css.artist}
                            />
                            <button className={css.addButton} onClick={() => queueSong(result)}>
                              <AddIcon />
                            </button>
                          </ListItem>
                        ))}
